...
ZMOD::ZMOD(uintptr_t baseAddress, uintptr_t dmaAddress, uintptr_t iicAddress, uintptr_t flashAddress,
	...
	printf("Entering constructor: ZMOD\n");
	printf("[DEBUG] Initializing ZMOD: Base Address: %p, DMA Address: %p, IIC Address: %p, Flash Address: %p, Direction: %d\n",
		   (void *)baseAddress, (void *)dmaAddress, (void *)iicAddress, (void *)flashAddress, direction);
	...
	printf("[DEBUG] ZMOD Initialized: Base Addr: %p, DMA Addr: %p, Flash Addr: %p\n", (void *)baseAddr, (void *)dmaAddr, (void *)flashAddr);
...
*/
ZMOD::~ZMOD() {
	printf("Entering destructor: ~ZMOD\n");
	printf("[DEBUG] Destroying ZMOD: Base Addr: %p, DMA Addr: %p, Flash Addr: %p\n", (void *)baseAddr, (void *)dmaAddr, (void *)flashAddr);
...
void ZMOD::writeReg(uint8_t regAddr, uint32_t value) {
	printf("Entering method: ZMOD::writeReg\n");
	printf("[DEBUG] Writing Register: Addr: %u, Value: %u\n", regAddr, value);
...
uint32_t ZMOD::readReg(uint8_t regAddr) {
	// printf("Entering method: ZMOD::readReg\n");
	uint32_t value = fnReadReg(baseAddr, regAddr);
	// printf("[DEBUG] Reading Register: Addr: %u, Value: %u\n", regAddr, value);
	return value;
}
...
void ZMOD::writeRegFld(uint8_t regAddr, uint8_t lsbBit, uint8_t noBits, uint32_t value) {
	printf("Entering method: ZMOD::writeRegFld\n");
	printf("[DEBUG] Writing Register Field: Addr: %u, LSB: %u, Bits: %u, Value: %u\n", regAddr, lsbBit, noBits, value);
...
uint32_t ZMOD::readRegFld(uint8_t regAddr, uint8_t lsbBit, uint8_t noBits) {
	// printf("Entering method: ZMOD::readRegFld\n");
	...
	// printf("[DEBUG] Reading Register Field: Addr: %u, LSB: %u, Bits: %u, Value: %u\n", regAddr, lsbBit, noBits, value);
	return value;
}
...
void ZMOD::writeSignedRegFld(uint8_t regAddr, uint8_t lsbBit, uint8_t noBits, int32_t value) {
	printf("Entering method: ZMOD::writeSignedRegFld\n");
...
int32_t ZMOD::readSignedRegFld(uint8_t regAddr, uint8_t lsbBit, uint8_t noBits) {
	// printf("Entering method: ZMOD::readSignedRegFld\n");
...
int32_t ZMOD::toSigned(uint32_t value, uint8_t noBits) {
	printf("Entering method: ZMOD::toSigned\n");
...
void* ZMOD::allocDMABuffer(size_t size) {
	printf("Entering method: ZMOD::allocDMABuffer\n");
	printf("[DEBUG] Allocating DMA Buffer: Size: %zu\n", size);
	return fnAllocUdmaBuf(dmaAddr, size);
}
...
void ZMOD::freeDMABuffer(uint32_t *buf, size_t size) {
	printf("Entering method: ZMOD::freeDMABuffer\n");
	printf("[DEBUG] Freeing DMA Buffer: Buf: %p, Size: %zu\n", buf, size);
	return fnFreeUdmaBuf(dmaAddr, buf, size);
}
...
void ZMOD::setTransferSize(size_t size) {
	printf("Entering method: ZMOD::setTransferSize\n");
	printf("[DEBUG] Setting Transfer Size: %zu\n", size);
	transferSize = size;
...
int ZMOD::startDMATransfer(uint32_t* buffer) {
	printf("[DEBUG] Starting DMA Transfer: Buffer: %p, Transfer Size: %zu\n", buffer, transferSize);
	// transfer length is not configured
	if (transferSize < 1) {
		return ERR_FAIL;
	}

	return fnStartDMATransfer(dmaAddr, buffer, transferSize);
}
...
bool ZMOD::isDMATransferComplete() {
	bool complete = fnIsDMATransferComplete(dmaAddr);
	return complete;
}
...
void ZMOD::sendCommand(uint32_t command) {
	printf("Entering method: ZMOD::sendCommand\n");
	printf("[DEBUG] Sending Command: %u\n", command);
...
uint32_t ZMOD::receiveCommand() {
	uint32_t command = readReg(ZMOD_REG_ADDR_CMD_RX);
...
void ZMOD::sendCommands(uint32_t *commands, size_t length) {
	printf("Entering method: ZMOD::sendCommands\n");
	if (!commands) {
		return;
	}

	printf("[DEBUG] Sending Commands: Length: %zu\n", length);
...
size_t ZMOD::receiveCommands(uint32_t *commands) {
	...
	printf("[DEBUG] Receiving Commands: Length: %zu\n", length);
...
size_t ZMOD::sendReceiveCommands(uint32_t *sentCommands, uint32_t *receivedCommands, size_t sentCommandsLength) {
	printf("Entering method: ZMOD::sendReceiveCommands\n");
	printf("[DEBUG] Sending and Receiving Commands: Sent Length: %zu\n", sentCommandsLength);
	...
	printf("[DEBUG] Received Commands Length: %zu\n", receivedCommandsLength);
...
{
	printf("[DEBUG] Reading User Calibration\n");
	int status;
	uint8_t crc;
	...
	printf("[DEBUG] User Calibration Status: %d\n", status);
	return status;
}
...
{
	printf("Entering method: ZMOD::initCalib\n");
	printf("[DEBUG] Initializing Calibration: Size: %u, ID: %u\n", calibSize, calibID);
	...
	int status = readUserCalib();
	printf("[DEBUG] Calibration Initialized: Status: %d\n", status);
...
{
	printf("[DEBUG] Writing User Calibration\n");
	uint8_t crc;
...
{
	printf("[DEBUG] Restoring Factory Calibration\n");
	...
	printf("[DEBUG] Factory Calibration Restored: Status: %d\n", status);
	return status;
}
...
{
	printf("Entering method: ZMOD::computeCRC\n");
	printf("[DEBUG] Computing CRC: Length: %u\n", len);
	uint8_t s = 0;
	uint32_t i;
	for(i = 0; i < len; i++)
	{
...
{
	printf("Entering method: ZMOD::formatValue\n");
	if (val < 0)
	{
