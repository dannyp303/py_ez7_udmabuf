...
{
	printf("Entering constructor: ZMODADC1410\n");
	printf("[DEBUG] Initializing ZMODADC1410: Base Address: %p, DMA Address: %p, IIC Address: %p, Flash Address: %p\n",
		   (void *)baseAddress, (void *)dmaAddress, (void *)iicAddress, (void *)flashAddress);
...
unsigned long long ZMODADC1410::allocChannelsBuffer(size_t &length) {
	printf("Entering method: ZMODADC1410::allocChannelsBuffer\n");
	if(length > ZMODADC1410_MAX_BUFFER_LEN)
	{
		length = ZMODADC1410_MAX_BUFFER_LEN;
	}
	printf("[DEBUG] Allocating Channels Buffer: Length: %zu\n", length);
	uint32_t *buf = (uint32_t *)ZMOD::allocDMABuffer(length * sizeof(uint32_t));
	printf("[DEBUG] Channels Buffer Allocated: Address: %p\n", (void *)buf);
...
void ZMODADC1410::freeChannelsBuffer(unsigned long long ptr, size_t length) {
	printf("Entering method: ZMODADC1410::freeChannelsBuffer\n");
	uint32_t *buf = (uint32_t*)ptr;
	printf("[DEBUG] Freeing Channels Buffer: Address: %p, Length: %zu\n", (void *)buf, length);
...
{
	printf("Entering method: ZMODADC1410::setTrigger\n");
	printf("[DEBUG] Setting Trigger: Channel: %u, Mode: %u, Level: %d, Edge: %u, Window: %u\n", channel, mode, level, edge, window);
...
void ZMODADC1410::setTransferLength(size_t &length) {
	printf("Entering method: ZMODADC1410::setTransferLength\n");
	if(length > ZMODADC1410_MAX_BUFFER_LEN)
	{
		length = ZMODADC1410_MAX_BUFFER_LEN;
	}
	printf("[DEBUG] Setting Transfer Length: %zu\n", length);
...
{
	printf("Entering method: ZMODADC1410::channelData\n");
	uint16_t channel_data = (channel ? (data >> 2) : (data >> 18)) & 0x00003FFF;
	printf("[DEBUG] Channel Data: Channel: %u, Data: %u, Extracted: %u\n", channel, data, channel_data);
	return channel_data;
}
...
int16_t ZMODADC1410::signedChannelData(uint8_t channel, uint32_t data) {
	printf("Entering method: ZMODADC1410::signedChannelData\n");
	int16_t signed_data = toSigned(channelData(channel, data), 14);
	printf("[DEBUG] Signed Channel Data: Channel: %u, Data: %u, Extracted: %d\n", channel, data, signed_data);
	return signed_data;
}
...
{
	printf("Entering method: ZMODADC1410::acquirePolling\n");
	printf("[DEBUG] Acquiring Polling: Channel: %u, Mode: %u, Level: %u, Edge: %u, Window: %u, Length: %zu\n", channel, mode, level, edge, window, length);
	int rc;
	...
	if (rc) {
		printf("[DEBUG] DMA Transfer Failed\n");
		return ERR_FAIL;
	}
	printf("Waiting for DMA to Complete transfer in acquirePolling\n");
    // Wait for DMA to Complete transfer
	// Assuming dmaAddr is a pointer to DMAEnv
    while(!isDMATransferComplete()) {}
	printf("DMA Transfer Completed Successfully in acquirePolling\n");

    printf("[DEBUG] DMA Transfer Completed Successfully\n");
    return ERR_SUCCESS;
}
...
{
	uint8_t buffer_full = readRegFld(ZMODADC1410_REGFLD_SR_BUF_FULL);
	// printf("[DEBUG] Buffer Full Status: %u\n", buffer_full);
	return buffer_full;
}
...
{
	printf("Entering method: ZMODADC1410::enableBufferFullInterrupt\n");
...
uint8_t ZMODADC1410::acquireTriggeredPolling(unsigned long long ptr, uint8_t channel, uint32_t level,
		uint32_t edge, uint32_t window, size_t length)
{
	printf("Entering method: ZMODADC1410::acquireTriggeredPolling\n");
...
{
	printf("Entering method: ZMODADC1410::acquireImmediatePolling\n");
...
uint8_t ZMODADC1410::acquireInterrupt(uint32_t* buffer, uint8_t channel, uint8_t mode, uint32_t level,
		uint32_t edge, uint32_t window, size_t length)
{
	printf("Entering method: ZMODADC1410::acquireInterrupt\n");
    ...
    start();
	printf("Waiting for DMA to Complete transfer in acquireInterrupt\n");
    // Wait for DMA to Complete transfer
    while(!isDMATransferComplete()) {}
...
uint8_t ZMODADC1410::acquireTriggeredInterrupt(unsigned long long ptr, uint8_t channel, uint32_t level,
		uint32_t edge, uint32_t window, size_t length)
{
	printf("Entering method: ZMODADC1410::acquireTriggeredInterrupt\n");
...
{
	printf("Entering method: ZMODADC1410::acquireImmediateInterrupt\n");
...
{
	printf("Entering method: ZMODADC1410::autoTestRamp\n");
	uint32_t *buf;
...
{
	printf("Entering method: ZMODADC1410::setGain\n");
	if(channel)
	{
...
{
	printf("Entering method: ZMODADC1410::setCoupling\n");
	if(channel)
	{
...
{
	printf("Entering method: ZMODADC1410::setCalibValues\n");
	CALIBECLYPSEADC *pCalib;
	if(calib)
	{
		// interpret the calib data as a CALIBECLYPSEADC data
...
{
	printf("Entering method: ZMODADC1410::computeCoefMult\n");
...
{
	printf("Entering method: ZMODADC1410::computeCoefAdd\n");
...
{
	printf("Entering method: ZMODADC1410::getVoltFromSignedRaw\n");
...
uint32_t ZMODADC1410::getData(unsigned long long ptr, int index) {
	printf("Entering method: ZMODADC1410::getData\n");
	uint32_t *buf = (uint32_t*)ptr;
	return buf[index];
}
